...
| 24 | 6 consumer files | `work_analysis` write-only, consumers all used raw `work.content` (P0 wasted token + logic bug) | P0 | **FIXED** — added `getWorkAnalysis()` to `_utils.js`; exam-preview, grade-preview, multiverse-preview, multiverse, chat, ai-exam-gen now use structured analysis |
| 25 | Library.tsx | Tab "Chunks" + `ChunksTab` component unused (no consumer reads `work_chunks`) | P1 | **FIXED** — removed `chunks` from `ANALYSIS_TABS`, removed `ChunksTab` component, `chunks` state, `useSWR` chunks, `chunkCount` badge; build clean |
| 26 | analyze.js:156 | `results.findIndex(r => r === result)` wrong scope — references `analysisSections` not `results` | P1 | **FIXED** — used `results.findIndex(r => r === result)` on outer `results` (Promise results), not inner `analysisSections`; error logging now correct |
...
- 2026-04-05 (session 5): ExamBank nâng cao — mode nhập tay với ExamManualForm, form tách rõ bài tập vs đề thi (lớp/thời lượng bắt buộc), exams.js hỗ trợ add questions; AddWorkModal AI suggestion banner (debounce 3s); Library.tsx polling sau analyze; build clean, deployed.
