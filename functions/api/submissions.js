  ...
  try {
    ...
    if (user.role === 'teacher') {
      const [rowsResult, countResult] = await Promise.all([
        env.DB.prepare(
          "SELECT s.id, s.exam_id AS examId, s.student_id AS studentId, s.status, s.ai_score AS aiScore, s.teacher_score AS teacherScore, s.teacher_comment AS teacherComment, s.submitted_at AS submittedAt FROM submissions s JOIN exams e ON s.exam_id = e.id WHERE e.teacher_id = ? ORDER BY s.submitted_at DESC LIMIT ? OFFSET ?"
